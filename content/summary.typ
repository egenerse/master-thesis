...
All requirements defined in Section 4.2 and their status are listed in @statusTable. (I : Implemented, HI: Half Implemented, N: Not Implemented)
...
[FR2], [Diagram Functionality], [HI],
[FR3], [Full Diagram Type Support], [HI],
[FR4], [Client-Side WebSocket Management for Collaborative Editing], [I],

[QA1], [Unified Monorepo Structure], [I],
[QA2], [Usability], [HI],
[QA3], [Maintainability], [I],
[QA4], [Performance], [I],
[QA5], [Accessibility], [I],
[QA6], [Mobile Application Support], [I],
[QA7], [Collaboration Traﬃc Limit], [I],
...
We successfully implemented the Apollon Reengineering Library along with the Webapp and Server components of Apollon Standalone. This fully satisfies QA1 (Unified Monorepo Structure) and partially fulfills FR1 (Artemis Compatibility).

The reengineered library offers comprehensive modeling features, including the ability to add, remove, and edit nodes and edges. Nodes can be moved and resized, while edges support reconnection. The library currently supports several diagram types, such as class, object, activity, component, and use case diagrams, but it does not yet cover all available types. As a result, FR3 (Full Diagram Type Support) is only partially satisfied.

The modeling experience is further enhanced by an infinite canvas, smoother and more intuitive zooming and panning, and the ability to navigate the canvas while dragging elements. These improvements contribute to a significantly better and more user-friendly interaction overall satisfies QA2 (Usability). Additionally, the library provides undo/redo functionality, which is essential for a smooth and flexible modeling workflow.

The library exposes a consistent API for importing diagrams in JSON format and exporting them as JSON, SVG, PNG, and PDF files, partially satisfying FR2 (Diagram Functionality).

The Webapp automatically stores the most recently edited diagram in local storage and includes a "load diagram" feature, allowing users to easily resume work on previously saved diagrams to attribute QA2 (Usability).
...
Additionally, we integrated Capacitor into the mono-repo, enabling us to deploy the latest changes directly to native iOS and Android apps. Touch-based events and gestures were handled carefully to improve the mobile experience across both apps and browsers. These improves QA5 (Acesibility).

We reintroduced collaboration mode using the same WebSocket-based infrastructure, fulfilling QA7 (Collaboration Traﬃc Limit). We ensured the collaboration traffic stays well below the 200 KB threshold currently under 100 KB. This demonstrates that the system can support larger collaboration sessions which fulfills QA4 (Performance)
...
FR1 (Artemis Integration) has not yet reached full completion. The drag-and-drop feedback functionality is still missing, primarily because the system lacks proper support for tracking highlighted elements. In order to assign feedback to individual diagram elements, especially attributes or methods within class diagrams, we need to implement a mechanism that tracks which parts of the diagram users highlight. Responsiveness and resizing of Modeling Editor is not implemented, which increase usability in the Artemis side.

Migration Script for old Apollon diagram JSON to the new format is not implemented. This is important for existing diagrams in Artemis, as they need to be converted to the new format to ensure rendering the diagrams with the reengineered library. 

Athena playground and converting to simplified modeling format using the new format is not implemented.
...
We could not yet implemented all planned usability features such as simplified sidebar and navigation bar. These missing components limit our ability to fully satisfy QA2 (Usability).

In terms of accessibility, we still need to address multiple areas. We we need to make edge creation better and easier for mobile users while making icons and handles bigger.By addressing these accessibility improvements, we will make the tool more inclusive and bring us closer to meeting QA5 (Accessibility) in future iterations.
...
Another valuable improvement would be mobile deep linking for collaboration, allowing users to open specific diagrams directly via shared links. Supporting different connection types such as edit, collaborate, give_feedback, and see_feedback through deep links or input fields (e.g., by entering a diagram ID) would make it easier to join the correct session with the intended permissions, improving accessibility and flexibility across both web and mobile.
